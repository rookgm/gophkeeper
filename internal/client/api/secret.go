...
func (c *Client) UpdateSecret(ctx context.Context, id uuid.UUID, token string) (*models.SecretResponse, error) {
	// PUT /api/user/secrets/{id}
	resp := &models.SecretResponse{}
	err := c.doRequest(ctx, "PUT", "/api/user/secrets/"+id.String(), &token, nil, resp)
	if err != nil {
		return nil, fmt.Errorf("create secret failed: %w", err)
	}
	return resp, nil
